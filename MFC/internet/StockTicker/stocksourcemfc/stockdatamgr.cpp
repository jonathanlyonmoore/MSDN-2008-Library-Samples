...
static const int SymbolFilePathSize = _MAX_PATH + _MAX_FNAME + sizeof(szSymbolFileName)+sizeof(TCHAR);
static TCHAR    szSymbolFilePath[SymbolFilePathSize] = _T("\0");
...
void CStockData::SetFromString(TCHAR* szStockData)
{
	...
	p = _tcstok_s(psz, _T(";"), &nextTok);
	ASSERT(p);  // should find at least 1.

	m_strSymbol = p;
	p = _tcstok_s(0, _T(";"), &nextTok);
	ASSERT(p);
	m_fCur = _tcstod(p, 0);
	p = _tcstok_s(0, _T(";"), &nextTok);
	ASSERT(p);
...
void CStockDataMgr::SetDataFilePath()
{
	...
	_tcscat_s(szSymbolFilePath, SymbolFilePathSize, szSymbolFileName);
	_tcscat_s(szSymbolFilePath, SymbolFilePathSize, _T("\0"));
}
