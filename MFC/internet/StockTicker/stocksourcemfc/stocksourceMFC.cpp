...
BOOL CStockSourceMFCExtension::GetExtensionVersion(HSE_VERSION_INFO* pVer)
{
	...
	TCHAR sz[sizeof(pVer->lpszExtensionDesc)/sizeof(*(pVer->lpszExtensionDesc))+1];
	sz[sizeof(pVer->lpszExtensionDesc)/sizeof(*(pVer->lpszExtensionDesc))]=_T('\0');
	ISAPIVERIFY(::LoadString(AfxGetResourceHandle(),
			IDS_SERVER, sz, sizeof(pVer->lpszExtensionDesc)/sizeof(*(pVer->lpszExtensionDesc))));
											 ...
											 LPCTSTR pstrString)
{
	...
	_stprintf_s(numBuf1, 20, "%.3f", sd.m_fCur);
	_stprintf_s(numBuf2, 20, "%.3f", sd.m_fChange);
