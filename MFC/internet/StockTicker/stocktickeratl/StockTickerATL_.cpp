...
LRESULT CStockTickerATL::OnStocks(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	if (stocksDialog.DoModal() == IDOK)
	{
		...
		for (; i1 != e1; i1++)
		{
			if ((tmpStr=T2OLE_EX((LPTSTR)(LPCTSTR)*i1,_ATL_SAFE_ALLOCA_DEF_THRESHOLD))==NULL)
				return 0;
			m_pStockQuotes->Add(tmpStr);
		}

		for (; i2 != e2; i2++)
		{
			if ((tmpStr=T2OLE_EX((LPTSTR)(LPCTSTR)*i2,_ATL_SAFE_ALLOCA_DEF_THRESHOLD))==NULL)
				return 0;
			m_pStockQuotes->Remove(tmpStr);
		}
		m_pStockQuotes->StartUpdating(m_updateInterval);
