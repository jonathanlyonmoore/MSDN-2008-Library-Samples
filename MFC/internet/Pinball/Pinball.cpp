...
BOOL CPinballExtension::GetExtensionVersion(HSE_VERSION_INFO* pVer)
{
	...
	BOOL ec = CHttpServer::GetExtensionVersion(pVer);
	if (ec == FALSE)
		return FALSE;

	// Load description string
	TCHAR sz[HSE_MAX_EXT_DLL_NAME_LEN];
	if(::LoadString(AfxGetResourceHandle(),
			IDS_SERVER, sz, HSE_MAX_EXT_DLL_NAME_LEN) == 0)
	{
		return FALSE;
	}

	_tcscpy_s(pVer->lpszExtensionDesc, HSE_MAX_EXT_DLL_NAME_LEN, sz);
	return TRUE;
}
	...
	else
	{
		...
		else
		{
			...
			if (hGlob != NULL)
			{
				...
				if (lpData != NULL)
				{
					memcpy_s(pszStorage, dwSize, lpData, dwSize);
					bResult = TRUE;
				}
...
void CPinballExtension::GetImage(CHttpServerContext* pCtxt, long dwChoice)
{
	...
	if (pCtxt->GetServerVariable("URL", szBuffer, &dwSize) == FALSE)
	{
		StartContent(pCtxt);
		WriteTitle(pCtxt);
		*pCtxt << _T("Trouble getting information from GetServerVariable.");
		*pCtxt << _T(" Call GetLastError for more info.<br>");
		EndContent(pCtxt);
	}
	...
	else
	{
		...
		dwSize = _MAX_PATH;
		*pstrWhack = NULL;
		if (pCtxt->ServerSupportFunction(HSE_REQ_MAP_URL_TO_PATH, szBuffer, &dwSize, 0) == NULL)
		{
			StartContent(pCtxt);
			WriteTitle(pCtxt);
			*pCtxt << _T("Trouble converting to local path.<br>");
			EndContent(pCtxt);
			return;
		}

		// make sure the buffer can hold full local path
		if (dwSize >= _MAX_PATH - 1)
		{
			StartContent(pCtxt);
			WriteTitle(pCtxt);
			*pCtxt << _T("Local path ");
			*pCtxt << szBuffer;
			*pCtxt << _T(" is too long.<br>");
			EndContent(pCtxt);
			return;
		}

		_tcscat_s(szBuffer, _MAX_PATH, "/");
