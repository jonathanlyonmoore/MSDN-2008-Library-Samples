...
size_t ArgParser::GetAttribute(LPCSTR name, StringList& values)
{
	ASSERT(name != NULL);
	if (name == NULL)
	{
	...
	size_t attributeCount = params.GetCount();	
	if (attributeCount == 0)
	{
...
size_t ArgParser::GetAttribute(LPCSTR name, CStringA& value)
{
	ASSERT(name != NULL);
	if (name == NULL)
	{
...
bool ArgParser::Parse(LPCSTR rawCmd)
{
	...
	{
		// skip whitespace
		while (rawCmd != NULL && isspace((unsigned char)*rawCmd))
			rawCmd++;
