...
CAtlRegExp<CharTraits> atlRegExp;
	...
	*/
	HRESULT SubstituteVars(CString &original,CAtlREMatchContext<CharTraits> &atlRematchContext){
		...
		HRESULT hr=S_OK;
		char *buffer;
		size_t bufSize = (atlRematchContext.m_uNumGroups/10)+2;
		buffer = new char[bufSize];
		CString temp,variableid;
		UINT i,y;
		for(i=1;i<=atlRematchContext.m_uNumGroups;i++){
			_itoa_s(i,buffer,bufSize,10);
			temp="";
			...
			if(hr!=S_OK&&hr!= MAKE_HRESULT(1,FACILITY_ITF,S_NO_MATCH))
				return hr;
		}
		delete[] buffer;
		...
		if(atlRegExp.Parse(oldexp,casesensitive)==REPARSE_ERROR_OK)
			if(	atlRegExp.Match(baseStr,&atlRematchContext)){
				...
				return S_OK;
				 	...
				 	return MAKE_HRESULT(0,FACILITY_ITF,S_NO_MATCH);

		else
			return MAKE_HRESULT(1,FACILITY_ITF,S_PARSE_FAILED); 
