...
namespace VCUE
{
	...
	public:
		...
		GetHttpStatusString(DWORD dwHttpStatus) throw()
		{
			...
			if (szStatusText)
			{
				int nRet = _snprintf_s(m_szStatus, sizeof(m_szStatus)/sizeof(CHAR), sizeof(m_szStatus)/sizeof(CHAR), "%d %s", dwHttpStatus, szStatusText);
				if (nRet == -1)
				{
					strcpy_s(m_szStatus + sizeof(m_szStatus) - 4, 4, "...");
				}
			}
		}
