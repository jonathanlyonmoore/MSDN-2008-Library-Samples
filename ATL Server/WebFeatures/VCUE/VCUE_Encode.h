...
namespace VCUE
{
	...
	{
		return ATL::EscapeToCString(strDestination, strSource);
	}
	...
	inline bool Base64Encode(const CStringA& strSource, CStringA& strDestination)
	{
		...
		int nBytes = ATL::Base64EncodeGetRequiredLength(strSource.GetLength(), ATL_BASE64_FLAG_NOPAD | ATL_BASE64_FLAG_NOCRLF);
		if (ATL::Base64Encode(
	...
	{
		bool bSuccess = false;
		int nBytes = ATL::Base64DecodeGetRequiredLength(strSource.GetLength());
		if (ATL::Base64Decode(
			strSource,
			strSource.GetLength(),
