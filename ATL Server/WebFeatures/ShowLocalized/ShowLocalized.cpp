...
public:
	...
	bool WriteLocaleInfo(LCID lcid)
	{
		...
		LPCSTR lpszRfc1766 = W2CA_EX(info.wszRfc1766, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
		LPCSTR lpszLocaleName = W2CA_EX(info.wszLocaleName, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
		
		if(lpszRfc1766 == NULL || lpszLocaleName == NULL)
			return false;

		if (SUCCEEDED(hr))
			m_HttpResponse << "<tr><td>" << info.lcid << "</td><td>" << lpszRfc1766 << "</td><td>" << lpszLocaleName << "</td></tr>\n";
		else
