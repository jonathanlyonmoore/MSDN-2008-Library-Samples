...
#define MAX_MULTIPLE_PARAM_SIZE 64
typedef struct tagMultipleParam
{
	char		_szWho[MAX_MULTIPLE_PARAM_SIZE];
	char		_szProduct[MAX_MULTIPLE_PARAM_SIZE];
	char		_szPlatform[MAX_MULTIPLE_PARAM_SIZE];
	int			_paramCount;
}stMultipleParam;
...
public:
	...
	*/
	...
	{
		if (NULL == pMemMgr || NULL == szParams || NULL == ppDest)
		{
			return HTTP_FAIL;
		}
	...
	*/
	...
	HTTP_CODE OnMultipleParamHello(stMultipleParam	*inputData)
	{
		...
		CStringA		szOutput;
		inputData->_szWho[MAX_MULTIPLE_PARAM_SIZE - 1] = '\0';
		inputData->_szProduct[MAX_MULTIPLE_PARAM_SIZE - 1] = '\0';
		inputData->_szPlatform[MAX_MULTIPLE_PARAM_SIZE - 1] = '\0';
	...
	*/
	HTTP_CODE parseMultipleHelloParam(IAtlMemMgr *pMemMgr, LPCSTR szParams,	stMultipleParam	**ppDest)
	{
		...
		*ppDest	=	(stMultipleParam*)pMemMgr->Allocate(iAllocSize);
		...
		if( iCommaPos > 0 )
		{
			...
			strncpy((*ppDest)->_szWho, szData, MAX_MULTIPLE_PARAM_SIZE - 1);
			(*ppDest)->_szWho[MAX_MULTIPLE_PARAM_SIZE - 1] = '\0';
		}
		...
		if( iCommaPos > 0 )
		{
			...
			strncpy((*ppDest)->_szProduct, szData, MAX_MULTIPLE_PARAM_SIZE - 1);
			(*ppDest)->_szProduct[MAX_MULTIPLE_PARAM_SIZE - 1] = '\0';
		}
		...
		if( iCommaPos > 0 )
		{
			...
			strncpy((*ppDest)->_szPlatform, szData, MAX_MULTIPLE_PARAM_SIZE - 1);
			(*ppDest)->_szPlatform[MAX_MULTIPLE_PARAM_SIZE - 1] = '\0';
		}
	...
	HTTP_CODE getNextFile(LPSTR	strExt)
	{
		...
		strncpy(buffFileName, m_HttpRequest.GetScriptPathTranslated(), MAX_PATH - 1);
		buffFileName[MAX_PATH - 1] = '\0';
		...
		strcat_s(buffFileName, MAX_PATH, strExt);
		buffFileName[MAX_PATH - 1] = '\0';
