...
public:
    BOOL GetFilterVersion(HINSTANCE hFilterDll, HTTP_FILTER_VERSION* pVer) throw()
    {
		...
		_ATLTRY
		{
			strcpy_s(pVer->lpszFilterDesc, sizeof(pVer->lpszFilterDesc), "Friendly URL remapping filter");
			pVer->dwFlags =
			...
			DWORD dwLen = GetModuleFileName(hFilterDll, CStrBufA(path, MAX_PATH+1), MAX_PATH+1);
...
private:
	...
	ATLENSURE(dwLen >= 0 && dwLen < dwLen+1);
        CHeapPtr<char> szBuf;
    ...
    {
		if (pfc == NULL || pfph == NULL)
		{
			SetLastError(SF_STATUS_REQ_ERROR);
