...
public:	
	...
	{
		BOOL bRet;
		bRet = ::SetEvent( m_hStopEvent );
		bRet = bRet && (::closesocket(m_listenSocket) != 0);
		bRet = bRet && (::WSACleanup() != 0);
		...
		bRet = bRet && ::CloseHandle( m_hStopEvent );
	...
	bool	StartListening(unsigned short nPort)
	{
		...
		AddRef();
		HANDLE hThread = ::CreateThread(NULL, 0, AcceptProc, (LPVOID)this, 0, &dwThreadId);
        if (hThread == NULL)
        {
    		cout	<<	"Cannot create process with error code: "	<<	::GetLastError()	<<	endl;
            return false;
        }
...
protected:
	...
	*/
	...
	static DWORD __stdcall ClientListenerProc(LPVOID	lpParam)
	{
        ...
        int RetVal = ::closesocket(pConn->clientSocket);
		bContinue = (RetVal == 0);
		pConn->bClientDisconnected	=	true;
