...
public:
	...
	HRESULT SendRequest(LPCTSTR tszAction) throw()
	{
		...
		// Build the request
		{
			...
			if (!buffWork.Append( (LPCSTR)&dwSize, sizeof(DWORD)))
                return E_FAIL;

			if (!buffWork.Append( (LPCSTR)m_strURL, m_strURL.GetLength() + 1))
                return E_FAIL;
			if (!buffWork.Append( szAction, (int)strlen(szAction) + 1))
                return E_FAIL;
			if (!buffWork.Append( (LPCSTR)m_requestStream.m_str, m_requestStream.m_str.GetLength()))
                return E_FAIL;
		}
