...
public:
	...
	BOOL Initialize(wchar_t*		lpwszResponseQueue)
	{
        ...
        if (wcslen(lpwszResponseQueue) >= MAX_PATH) 
            return false;
		...
		if( bRet )
		{
			bRet = m_msgQueue.SetFormatName(lpwszQueueName);
            if (bRet)
            {
...
protected:
	bool	getMsgParams(CMQMessageProps *pProps, CAtlIsapiBuffer<>	&xmlReq, CStringA& strURL, CStringA&	strSoapAction, CComBSTR&	bstrResMQ) 
	{
		...
		if( bContinue )
		{
			...
			HRESULT Result = bstrResMQ.Append(lpwszData);
	...
	void processMessage(CMQMessageProps *pProps)
	{
		...
		if( bContinue )
		{
			...
			{
				bool Result = sender.SendSOAPResponse( xmlRes );
                ATLASSERT(Result);
			}
		}
	}
...
public:
	...
	bool	Initialize(wchar_t*	lpwszQueuePath)
	{
		...
		if (hr == (HRESULT)MQ_ERROR_QUEUE_EXISTS)
		{
	...
	static void QueueReceiveRequest(CMQQueue *pQueue)
	{
		...
		do
		{
			...
			if(hPeekRet == (HRESULT)MQ_ERROR_IO_TIMEOUT )
			{
				// No message. Sleeping...
				Sleep(500);
			}
