...
inline HRESULT AddNewAppointment(
		...
		LPCTSTR szDetails)
	{
		...
		if(FAILED(hr = StringCchPrintf(newAppointment.m_szDate, DB_MAX_STRLEN, _T("%d/%d/%d"), nMonth, nDay, nYear))) 
			return hr;
		if(FAILED(hr = StringCchCopy(newAppointment.m_szTitle, DB_MAX_STRLEN, szTitle)))
			return hr;
        if(FAILED(hr = StringCchCopy(newAppointment.m_szLocation, DB_MAX_STRLEN, szLocation)))
			return hr;
        if(FAILED(hr = StringCchCopy(newAppointment.m_szDetails, DB_MAX_DETAILSLEN, szDetails)))
			return hr;
...
public:
	...
	HTTP_CODE ValidateAndExchange()
	{
		...
		// If it is an invalid date, reset to today
		...
		if(FAILED(StringCchPrintf(m_appointmentList.m_szDate, DB_MAX_STRLEN, _T("%d/%d/%d"), m_date.GetMonth(), m_date.GetDay(), m_date.GetYear())))
          return  HTTP_FAIL;
...
private:
	bool ValidateFormData(const CHttpRequestParams& FormFields)
	{
		...
		if(FAILED(StringCchPrintf(findAppointment.m_szDate, DB_MAX_STRLEN, _T("%d/%d/%d"), m_date.GetMonth(), m_date.GetDay(), m_date.GetYear())))
			return false;
