...
private:
	...
	HTTP_CODE AddTask()
	{
		...
		// Copy the task data
		if(FAILED( StringCchCopy(newTask.m_szTitle, DB_MAX_STRLEN, m_strTaskTitle)))
            return HTTP_FAIL;

		newTask.m_dueDate = (DATE) m_date;
		newTask.m_lPriority = m_iPriority;
		if(FAILED(StringCchCopy(newTask.m_szDetails, DB_MAX_DETAILSLEN, m_strTaskDetails)))
			return HTTP_FAIL ;
	...
	HTTP_CODE DeleteMarkedTasks()
	{
		...
		while (pos != NULL)
		{
			...
			if (sscanf_s(varname, "taskid%d", &deleteTask.m_lTaskID) != 0)
			{
