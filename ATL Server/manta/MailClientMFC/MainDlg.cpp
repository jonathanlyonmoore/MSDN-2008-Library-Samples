...
void CMainDlg::OnSysCommand(UINT uCode, LONG_PTR lPos)
{
	...
	if (uCode == SC_MINIMIZE)
	{
		...
		nid.hIcon = m_hIcon;
		lstrcpyn(nid.szTip, "MantaWeb Mail Service Client",sizeof(nid.szTip));
...
void CMainDlg::OnCheckMail()
{
	...
	if (iMessageCount > 0)
	{
		...
		for (int i = 0; i < iOutMsgCount; i++)
		{
			...
			LPCTSTR pszFrom = CW2T(msgData[i].m_bstrFrom);
			LPCTSTR pszSubject = CW2T(msgData[i].m_bstrSubject);
			if(pszFrom == NULL || pszSubject == NULL)
			{
				delete[] msgData;
				MessageBox("Out of Memory.", "MantaWeb Mail Service Error", MB_OK | MB_ICONHAND);
				EnableCheckMailButton();			
				return;
			}

			m_ctrlMsgList.SetItemText(index, 1, pszFrom);
			m_ctrlMsgList.SetItemText(index, 2, pszSubject);
...
void CMainDlg::OnMessageDblClick(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	USES_CONVERSION_EX;
	LPCTSTR pszUrl = W2CT_EX(bstrURL,_ATL_SAFE_ALLOCA_DEF_THRESHOLD);
	if(pszUrl == NULL)
	{
		MessageBox("Out of memory.", "Mail Service Error", MB_OK | MB_ICONHAND);
		return;
	}

	// Open the browser to the url
	if (ShellExecute(NULL, "open", pszUrl, NULL, NULL, SW_SHOWNORMAL) < (HINSTANCE) 32)
	{
