...
// Note: the directory permissions for IUSR_<MACHINE NAME> should be set to read/write.
const char MANTAWEB_FILESTORE_ROOT[] = "C:\\MantaWebSample\\FileStore";
...
public:
	...
	HTTP_CODE ValidateAndExchange()
	{
		...
		{
			m_bCreateFolderError = !CreateNewFolder(FormFields.Lookup("curfolder"), FormFields.Lookup("folder"));
		}
		...
		m_strCurFolder = m_strCurFolder.Right(m_strCurFolder.GetLength() - m_strHomeFolder.GetLength());
...
protected:
	...
	bool CreateNewFolder(LPCSTR lpcszCurFolder, LPCSTR lpcszFolderName)
	{
		// Create a new folder
		CStringA strDirectory;

		if (lpcszCurFolder == NULL || lpcszFolderName == NULL)
			return false;

		if (!IsStringValid(lpcszCurFolder, true, true, " .\\") || !IsStringValid(lpcszFolderName, true, true, " ."))
			return false;
			
		strDirectory.Format("%s%s\\%s", m_strHomeFolder, lpcszCurFolder, lpcszFolderName);
		
		if (!ValidatePath(strDirectory))
			return false;

		return (CreateDirectory(strDirectory, NULL)) ? true : false;
	}
