...
public:
	HTTP_CODE ValidateAndExchange()
	{
		...
		if (!GetUserID(&msgCount.m_lUserID))
...
protected:
	HTTP_CODE DeleteMarkedMessages()
	{
		...
		if(!GetUserID(&markMsg.m_lUserID))
			return HTTP_FAIL;
	...
	HTTP_CODE SendMail()
	{
		...
		if(!GetUserID(&outboxMsgs.m_lUserID))
			return HTTP_FAIL;
	...
	HTTP_CODE SendInternalMessage(const COutboxMessages& outboxMsg)
	{
		...
		if(FAILED(StringCchCopy(findUserID.m_szLogin, DB_MAX_STRLEN, outboxMsg.m_szTo)))
			return HTTP_FAIL;
		...
		if(FAILED(StringCchCopy(addMessage.m_szTo, DB_MAX_STRLEN, outboxMsg.m_szTo)))
			return HTTP_FAIL;
		if(FAILED(StringCchCopy(addMessage.m_szSubject, DB_MAX_STRLEN, outboxMsg.m_szSubject)))
			return HTTP_FAIL;
		if(FAILED(StringCchCopy(addMessage.m_szFrom, DB_MAX_STRLEN, outboxMsg.m_szFrom)))
			return HTTP_FAIL;
		if(FAILED(StringCchCopy(addMessage.m_szMessage, MAX_MSG_LENGTH, outboxMsg.m_szMessage)))
			return HTTP_FAIL;
	...
	HTTP_CODE SendExternalMessage(const COutboxMessages& outboxMsg)
	{
		...
		strSender.Format("%s@MantaWeb.sample",  GetLogin());
