...
public:
	HTTP_CODE ValidateAndExchange()
	{
		...
		if (lstrcmp(QueryParams.Lookup("ref"), "inbox") == 0)
		{
			...
			if (!GetUserID(&markAsRead.m_lUserID))
				return HTTP_BAD_REQUEST;
		...
		if(!GetUserID(&m_messageInfo.m_lUserID))
			return HTTP_BAD_REQUEST;
...
protected:
	...
	HTTP_CODE SaveMessage()
	{
		...
		if(!GetUserID(&updateMsg.m_lUserID))
			return HTTP_FAIL;

		if (m_HttpRequest.GetFormVars().Exchange("message", &updateMsg.m_lMessageID) != VALIDATION_S_OK)
			return HTTP_FAIL;

		// Copy the new message text
		LPCSTR lpszMessage = m_HttpRequest.GetFormVars().Lookup("messageText");
		if(FAILED(StringCchCopy(updateMsg.m_szMessage, MAX_MSG_LENGTH, lpszMessage)))
			return 	HTTP_FAIL;
