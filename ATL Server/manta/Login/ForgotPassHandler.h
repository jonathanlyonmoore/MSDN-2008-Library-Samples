...
public:
	HTTP_CODE ValidateAndExchange()
	{
		...
		if (lpszLogin)
		{
			...
			if(FAILED(StringCchCopy(findEmail.m_szLogin, DB_MAX_STRLEN, lpszLogin)))
				return HTTP_FAIL;
...
protected:
	bool ValidateFormData(const CHttpRequestParams& FormFields)
	{
		...
		CUserInfo userInfo;
		if(FAILED(StringCchCopy(userInfo.m_szEmail, DB_MAX_STRLEN, m_strEmail)))
			return false;
