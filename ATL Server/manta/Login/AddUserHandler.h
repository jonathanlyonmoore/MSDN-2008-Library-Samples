...
protected:
	...
	bool ValidateFormData(const CHttpRequestParams& FormFields)
	{
		...
		if (FormFields.Validate("password1", &m_strPassword1, 1, DB_MAX_STRLEN) != VALIDATION_S_OK ||
			!IsStringValid(m_strPassword1, true, true, " "))
		{
			m_fFailure.m_bFailure = true;
		...
		if (FormFields.Validate("hint", &m_strHint, 1, DB_MAX_STRLEN) != VALIDATION_S_OK ||
			!IsStringValid(m_strHint, true, true, " "))
		{
			m_fFailure.m_bFailure = true;
			m_fFailure.m_bInvalidHint = true;
		}
		// Lookup the remember password check
		if (FormFields.Lookup("rememberme") != NULL)
		...
		if(FAILED(StringCchCopy(findUserID.m_szLogin, DB_MAX_STRLEN, m_strLogin)))
			return false;
	...
	HTTP_CODE ConfirmUser(const CStringA& strUser)
	{
		...
		if(FAILED(StringCchCopy(confirmUser.m_szLogin, DB_MAX_STRLEN, strUser)))
			return HTTP_FAIL;
	...
	HTTP_CODE AddUser()
	{
		...
		CAddUser addUser;
		if(FAILED(StringCchCopy(addUser.m_szFirstName, DB_MAX_STRLEN, m_strFirstName)))
			return HTTP_FAIL;
		if(FAILED(StringCchCopy(addUser.m_szLastName, DB_MAX_STRLEN, m_strLastName)))
			return HTTP_FAIL;
		if(FAILED(StringCchCopy(addUser.m_szEmail, DB_MAX_STRLEN, m_strEmail)))
			return HTTP_FAIL;
		if(FAILED(StringCchCopy(addUser.m_szLogin, DB_MAX_STRLEN, m_strLogin)))
			return HTTP_FAIL;
		if(FAILED(StringCchCopy(addUser.m_szHint, DB_MAX_STRLEN, m_strHint)))
			return HTTP_FAIL;
		addUser.m_bRememberPassword = (m_bRememberPass) ? VARIANT_TRUE : VARIANT_FALSE;
		CoCreateGuid(&addUser.m_guidRememberID);

		HRESULT hr;

		// Don't store the real password. only store a one-way encrypted hash
		addUser.m_lSaltLen = 4;
		addUser.m_lPassLen = DB_MAX_STRLEN;

		hr = HashSecret((const BYTE*) (LPCSTR) m_strPassword1, m_strPassword1.GetLength(),
						addUser.m_Salt, (DWORD &) addUser.m_lSaltLen,	addUser.m_Password, 
						(DWORD &) addUser.m_lPassLen);
		if (FAILED(hr))
			return HTTP_FAIL;
		...
		if (hr != S_OK)
		 ...
		 // Write the persistant cookie
		if (m_bRememberPass)
			WritePersistCookie(addUser.m_szLogin, addUser.m_szFirstName, addUser.m_guidRememberID);
		else
