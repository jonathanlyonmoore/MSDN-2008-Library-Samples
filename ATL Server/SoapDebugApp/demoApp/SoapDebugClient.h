...
public:
	...
	HRESULT __stdcall Read(void *pDest, ULONG nMaxLen, ULONG *pnRead) throw()
	{
		...
		if (nRead < m_nBodyLen)
		{
			ULONG nLength = min((m_nBodyLen - nRead), nMaxLen);
			memcpy_s(pDest, nLength, (LPCSTR)m_str + nRead, nLength);
			m_nCurr+= nLength;
...
public:
	...
	LPCSTR	GetFullDllPath()
	{
		...
		else
		{
			...
			if( m_dllPath[0] == '\\')
			{
				...
				strcpy_s(m_fullDllPath + 2, sizeof(m_fullDllPath) - 2, m_dllPath);
			...
			else
			{
				...
				sprintf_s(tempBuff, sizeof(tempBuff), curDir);
				sprintf_s(tempBuff + strlen(curDir), sizeof(tempBuff) - strlen(curDir), "\\");
				sprintf_s(tempBuff + strlen(tempBuff), sizeof(tempBuff) - strlen(tempBuff),m_dllPath);
#ifdef _DEBUG 
				BOOL	bCanonicalized =	::PathCanonicalizeA( m_fullDllPath, tempBuff);
				ATLASSERT(bCanonicalized);
#else
				::PathCanonicalizeA( m_fullDllPath, tempBuff);
#endif
			}
		}
		return m_fullDllPath;
	...
	HRESULT		SetSoapAppDllPath(LPCSTR		szFile)
	{
		...
		strncpy_s( m_dllPath, sizeof(m_dllPath), szFile, _TRUNCATE);
		m_dllPath[MAX_PATH] = 0;
		*m_fullDllPath	=	0;
		return S_OK;
	}
	...
	HRESULT		SetProtocol(LPCSTR		szProtocol)
	{
		...
		strncpy_s( m_szProtocol, sizeof(m_szProtocol), szProtocol, _TRUNCATE);
		m_szProtocol[MAX_PATH] = 0;
		return S_OK;
	}
	...
	void				SetUrl(LPCTSTR	tszURL)
	{
		...
		if( pQMark != NULL )
		{
			if( strlen(pQMark+1) < ATL_URL_MAX_URL_LENGTH )
				strncpy_s(m_szQueryString, sizeof(m_szQueryString), pQMark+1, _TRUNCATE);
		...
		if( *m_dllPath == '\0' )
		{
			...
			if( strlen(pAppStart) < MAX_PATH)
				strncpy_s(m_dllPath, sizeof(m_dllPath), pAppStart, _TRUNCATE );
		}
...
public:
	CSoapDebugClient(LPCTSTR strURL = NULL) : m_errorState(SOAPCLIENT_SUCCESS)
	{
		...
		*m_fullDllPath		= 0;
		...
		sprintf_s( m_szProtocol, sizeof(m_szProtocol), "HTTP/1.1");
		SetOutputFile(NULL);
		SetErrorFile(NULL);
		SetUrl(strURL);
...
protected:
	void	upcaseHttpHeaderName(CStringA&	strName)
	{
		...
		{
			if( bUpCase )
				strName.SetAt(iIndex, (char)toupper( strName[iIndex] ));
...
public:
	...
	void SetPathTranslated(LPCSTR szPath)
	{
		...
		strncpy_s(m_szPathTranslated, sizeof(m_szPathTranslated), szPath, _TRUNCATE);
		...
		memcpy_s(m_szScriptPathTranslated, sizeof(m_szScriptPathTranslated), m_szPathTranslated, dwResult);
		...
		DWORD * pdwSize) throw()
	{
		...
		if( _stricmp( pszVariableName, "ALL_RAW") == 0 )
		{
			// render all map
			POSITION	pos;
		...
		else if( _stricmp( pszVariableName, "URL") == 0 )
		{
		...
		else if( _stricmp( pszVariableName, "SERVER_NAME") == 0 )
		{
			strValue	=	"NoServer";
		}
		else if( _stricmp( pszVariableName, "SERVER_PROTOCOL") == 0 )
		{
			strValue	=	m_szProtocol;
		}
		else if( _stricmp( pszVariableName, "HTTPS") == 0 )
		{
			strValue	=	"off";
		}
		...
		{
			if(  pvBuffer )
				strncpy_s(pvBuffer, strValue.GetLength() + 1, strValue, _TRUNCATE);
		}
		else
		{
			if(  pvBuffer )
	...
	struct ServiceNode
	{
		...
		const ServiceNode& operator=(const ServiceNode& that) throw()
		{
			if (this != &that)
			{
				hInst = that.hInst;
				punk = that.punk;
				memcpy_s(&guidService, sizeof(GUID), &that.guidService, sizeof(guidService));
				memcpy_s(&riid, sizeof(IID), &that.riid, sizeof(riid));
			}
			return *this;
		}
	};
...
public:
	...
	HTTP_CODE LoadDispatchFile(LPCSTR szFileName, AtlServerRequest *pRequestInfo) throw()
	{
		...
		else
		{
			...
			pStencil->GetHandlerName(szDllPath, strlen(szDllPath), szHandlerName, strlen(szHandlerName));
	...
	virtual HRESULT AddService(REFGUID guidService, REFIID riid, IUnknown *punk, HINSTANCE hInstance) throw()
	{
		...
		srvNode.punk = punk;
		memcpy_s(&srvNode.guidService, sizeof(GUID), &guidService, sizeof(guidService));
		memcpy_s(&srvNode.riid, sizeof(IID), &riid, sizeof(riid));
...
inline HRESULT CSoapDebugClient::SendRequest(LPCTSTR tszAction) throw()
{
	...
	strcpy_s(m_szVerb, sizeof(m_szVerb), "POST");
	strcpy_s(m_szContentType, sizeof(m_szContentType), "text/xml\r\n");
	...
	if( pSoapAction )
	{
		...
		strcpy_s(buff, sizeof(buff), szAction);
		strSoapAction	=	buff;
	}
	else
		return E_FAIL;
