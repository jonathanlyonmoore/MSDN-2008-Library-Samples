...
// constants for the INI file and the maximum setting length
const TCHAR* HTTP_PING_INI = _T(".\\HttpPing.ini");
const DWORD MAX_SETTING_LEN = 128;

bool use_proxy = 0;
TCHAR proxyName[MAX_SETTING_LEN];
int proxyPort = 80;
...
void LoadSettings()
{
	...
	TCHAR str[MAX_SETTING_LEN];
	CString str_true;
	str_true.LoadString(IDS_TRUE);

	GetPrivateProfileString( _T("settings"), _T("use_proxy"), _T("false"), str, MAX_SETTING_LEN, HTTP_PING_INI );
	if (str_true.CompareNoCase(str) == 0)
		use_proxy = TRUE;
	else
		use_proxy = FALSE;

	GetPrivateProfileString( _T("settings"), _T("proxy"), _T(""), proxyName, MAX_SETTING_LEN, HTTP_PING_INI );

	GetPrivateProfileString( _T("settings"), _T("proxyport"), _T("80"), str, MAX_SETTING_LEN, HTTP_PING_INI );
	proxyPort = _ttoi(str);
}
