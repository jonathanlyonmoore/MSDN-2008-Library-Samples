...
public:
	...
	[db_column(ordinal = "1", status = "m_dwEmailStatus", length = "m_dwEmailLength")]
	char m_Email[256];
...
{
public:
	[db_column(ordinal = "1", status = "m_dwNameStatus", length = "m_dwNameLength")]
	char m_Name[51];

	[db_column(ordinal = "2", status = "m_dwDescriptionStatus", length = "m_dwDescriptionLength")]
	char m_Description[300];

	[db_column(ordinal = "3", status = "m_dwPriceStatus")]
	long m_Price;

	[db_column(ordinal = "4", status = "m_dwIdStatus")]
	long m_Id;
...
[
	db_command("SELECT [SiteUser].[Password], [SiteUser].[Salt] FROM [SiteUser] WHERE [SiteUser].[UserName] = ?")
]
class CSiteUser
{
public:
	...
	[ db_column(ordinal = "1", status = "m_dwPasswordStatus", length = "m_dwPasswordLength") ]
	BYTE m_Password[51];

	[ db_column(ordinal = "2", status = "m_dwSaltStatus", length = "m_dwSaltLength") ]
	BYTE m_Salt[4];
