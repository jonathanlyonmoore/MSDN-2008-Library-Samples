		...
		private void importOutlook_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				for(int i=1;i<=myContactsFolder.Items.Count;i++)
				{
					try
					{
						ContactList.Items.Add(((Outlook._ContactItem)myContactsFolder.Items[i]).FullName);
						ContactItemList[i-1-badItemsCounter]=new OnlineAddressBookServer.ContactItem();
						mapExchangeItemtoWSItem((Outlook._ContactItem)myContactsFolder.Items[i],ref ContactItemList[i-1-badItemsCounter]);
					}
