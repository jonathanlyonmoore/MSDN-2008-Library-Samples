...
__interface IPersistSoapServer
{
	/* 
		...
		It's up to the implementation whether the transport token is a cookie or a SOAP header
	*/
...
protected:
	storageKey		renderNewKey()
	{
		...
		char					buffChar[MAX_SESSION_KEY_LEN];
		storageKey				ret;
		if( SUCCEEDED(nameGen.GetNewSessionName(buffChar, &dwSize) ) ) 
		{
			ATLENSURE (dwSize < MAX_SESSION_KEY_LEN);
			buffChar[dwSize] = '\0';
			ret	=	buffChar;
		}
		...
		try
		{
			...
			if( !bNewObject )
			{
				...
				if( !bFound )
				{
					...
					hRet	=	MAKE_HRESULT(SEVERITY_ERROR, FACILITY_STORAGE, eOBJECT_NOT_FOUND);
				}
				else
				{
					...
					if (SUCCEEDED(hRet)) {
                        hRet	=	MAKE_HRESULT(SEVERITY_SUCCESS, FACILITY_STORAGE, eEXISTING_OBJECT);
					}
				}
			}
			else
			{
				...
				hRet = pServer->setObjToken( token );

				if (SUCCEEDED(hRet)) {
					hRet	=	MAKE_HRESULT(SEVERITY_SUCCESS, FACILITY_STORAGE, eNEW_OBJECT);
				}
			}
		}
		catch(...)
