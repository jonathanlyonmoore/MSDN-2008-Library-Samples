...
inline HRESULT STDMETHODCALLTYPE CPerfObjectDisp::GetCounter(/* [in] */ int nCounter, /* [out, retval] */ VARIANT* pvValue)
{
	...
	CPerfMon::CounterInfo* pEntry = NULL;
	CPerfMon::CategoryInfo* pCat=m_pPerfMon->_FindCategoryInfo(m_pObject->m_dwCategoryId);
	if (pCat)
	{
		pEntry = m_pPerfMon->_FindCounterInfo(pCat, nCounter);
	}
	if (!pEntry)
		return E_FAIL;
...
)
{
	*pnPerfId = m_nNextPerfId;
	...
	HRESULT hr = CPerfMon::AddCategoryDefinition(
		(DWORD) m_nNextPerfId,
...
)
{
	...
	*pnPerfId = m_nNextPerfId;
	...
	CPerfMon::CategoryInfo* pCat=_GetCategoryInfo(nCategory);
	if (pCat)
	{
		pCat->m_nStructSize +=nSize;
		pCat->m_nAllocSize  +=nSize;
	}
	m_nNextPerfId++;
	...
	else
	{
		...
		while (*pszTemp)
		{
			...
			if (!*pszTemp)
			{
				...
				_tcscpy_s(pszTemp, dwChars - (pszTemp - pszBuf), m_strAppName);
...
inline HRESULT STDMETHODCALLTYPE CPerfMonDisp::GetObjectId(/* [in] */ BSTR bstrName, /* [out, retval] */ int* pnId)
{
	...
	*pnId = 0;
	...
	UINT nIndex = 0;
	while (nIndex < _GetNumCategories())
	{
		CPerfMon::CategoryInfo* pCat=_GetCategoryInfo(nIndex);
				
		if (pCat && strTarget == pCat->m_strName )
		{
			*pnId = (int) pCat->m_dwCategoryId;
			return S_OK;	
		}		
	}
...
inline HRESULT STDMETHODCALLTYPE CPerfMonDisp::GetCounterId(/* [in] */ int nObjectId, /* [in] */ BSTR bstrName, /* [out, retval] */ int* pnId)
{
	...
	*pnId = 0;
	...
	CPerfMon::CategoryInfo* pCat=_FindCategoryInfo(nObjectId);
	if (pCat)
	{
		for (ULONG i=0; i<pCat->_GetNumCounters(); i++)
		{
			CPerfMon::CounterInfo* pCounter=pCat->_GetCounterInfo(i);			
			if (strTarget == pCounter->m_strName)
			{
				*pnId = pCounter->m_dwCounterId;
				return S_OK;
			}
		}
	}
