...
BOOL CMFCMainFrame::Initialize(int cx, int cy)
{
	...
	CLSID clsidTangram;
	hr = ::CLSIDFromProgID(L"AtlTangram.AtlTangramModelAttrib", &clsidTangram);
	if (FAILED(hr))
	{
		ErrorMessage("Failed to create Tangram CLSID from ProgID.", hr) ;
		return FALSE ;
	}

	for (int i = 0 ; i < POLYGONS ; i ++)
	{
		hr = ::CoCreateInstance(clsidTangram,
								NULL,
								clsctx ,
		...
		hr = pITangramModel->QueryInterface(IID_IATLTangramTransform, (void**)&pITangramTransform ) ;
		ASSERT_HRESULT(hr) ;
...
void CMFCMainFrame::DoButtonDown(UINT nWhichButton, CPoint point)
{
	...
	if (hr == S_OK)
	{
		ATLENSURE(m_pSelectedVisual != NULL) ;
		//
